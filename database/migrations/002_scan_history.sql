...
-- GIN index for JSONB columns (for advanced queries)
...
-- Find regressed findings (appeared in new scan but were absent in previous)
-- WITH latest_two_scans AS (
...
--     LIMIT 2
-- )
-- SELECT f2.fingerprint, f2.title, f2.endpoint
-- FROM findings f2
-- JOIN latest_two_scans s2 ON s2.id = f2.scan_id
-- WHERE s2.created_at = (SELECT MAX(created_at) FROM latest_two_scans)
--   AND f2.fingerprint IN (
--       SELECT f1.fingerprint
--       FROM findings f1
--       JOIN latest_two_scans s1 ON s1.id = f1.scan_id
--       WHERE s1.created_at = (SELECT MIN(created_at) FROM latest_two_scans)
--   );
...
-- DROP VIEW IF EXISTS findings_with_scan_context;
-- DROP VIEW IF EXISTS recent_scans;
-- DROP FUNCTION IF EXISTS generate_finding_fingerprint(TEXT, TEXT, TEXT);
